export function renderTerms(container: HTMLElement) {
  container.innerHTML = `
    <div style="max-width: 800px; margin: 0 auto; padding: 40px 20px; font-family: 'Inter', 'Pretendard', sans-serif; line-height: 1.6; color: #e2e8f0;">
      ...
      <p style="color: #94a3b8; font-size: 0.9em; margin-top: 4px;"><strong>2. Description of Service:</strong> rhwp Studio is a web-based application that allows users to open and edit HWP files directly from their Google Drive. HWPX files are supported in preview (view-only) mode. The Service uses WebAssembly to process documents entirely within your browser.</p>
